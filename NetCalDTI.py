...
class NetCalDTI(torch.nn.Module):
    def __init__(self,k=128,calibration=True):
        super(NetCalDTI, self).__init__()
        self.calibration = calibration
        self.conv = 40
        self.k = k
        '''
        ***---Here we adjust the components of NetFuseDTI---***
        use_le:Network Regularization 
        '''
        ...
        self.le_mlp = mlp(k*2,512,128,0.5)
    ...
    def forward(self,emb1, emb2, seq, esm, smiles, mole):
        if not self.calibration:
            emb1 = torch.zeros_like(emb1)
            emb2 = torch.zeros_like(emb2)
