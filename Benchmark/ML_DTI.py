...
class ML_DTI(nn.Module):
    def __init__(self, vocab_prot_size=26, vocab_drug_size=65, embedding_size=128, filter_num=32):
        super().__init__()
    ...
    def forward(self, emb1, emb2, seq, esm, smiles, mole):
        prot_x = self.prot_embed(seq_emb_prot(seq)).permute(0, 2, 1)
        drug_x = self.drug_embed(seq_emb_drug(smiles)).permute(0, 2, 1)
